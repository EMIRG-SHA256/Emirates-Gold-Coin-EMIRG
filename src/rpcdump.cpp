// Copyright (c) 2009-2012 EmiratesGoldCoin Developers
...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <emiratesgoldcoinprivkey> [label] [rescan=true]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <emiratesgoldcoinaddress>\n"
            "Reveals the private key corresponding to <emiratesgoldcoinaddress>.");
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by EmiratesGoldCoin %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
